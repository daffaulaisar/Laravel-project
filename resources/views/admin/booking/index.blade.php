@extends('layouts.app', ['title' => 'booking - Admin'])
@section('content')
<main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-300">
    <div class="container mx-auto px-6 py-8">
        <div class="flex items-center"> <button class="text-white
        focus:outline-none bg-gray-600 px-4 py-2 shadow-sm rounded-md"> <a href="{{route('admin.booking.create')}}">
                    TAMBAH</a>
            </button>
            <div class="relative mx-4"> <span class="absolute inset-y-0 left-0 pl-3 flex itemscenter">
                ...
                <form action="{{ route('admin.booking.index') }}" method="GET"> <input class="form-input w-full
        ...
        <div class=" -mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
            <div class="inline-block min-w-full shadow-sm rounded-lg overflow-hidden">
                <table class="min-w-full table-auto">
                    <thead class=" justify-between">
                        <tr class="bg-gray-600 w-full">
                            <th class="px-16 py-2"> <span class="text-white">Customer</span> </th>
                            <th class="px-16 py-2 text-left">
                                <span class="text-white">Penerbangan</span>
                            </th>
                            <th class="px-16 py-2 text-left">
                                <span class="text-white">Waktu Booking</span>
                            </th>
                            <th class="px-16 py-2 text-left">
                                <span class="text-white">Bangku</span>
                            </th>
                            <th class="px-16 py-2 text-left">
                    ...
                    <tbody class="bg-gray-200">
                        ...
                        <tr class="border bg-white">
                            <td class="px-16 py-2">{{ $bk->customer->nama_depan}} {{ $bk->customer->nama_belakang}}</td>
                            <td class="px-16 py-2">{{ $bk->penerbangan->pesawat}}</td>
                            <td class="px-16 py-2">{{ $bk->waktu}}</td>
                            <td class="px-16 py-2">{{ $bk->bangku}}</td>
                            <td class="px-16 py-2">{{ $bk->total_hrg}}</td>

                            <td class="px-10 py-2 text-center">
                                <a href="{{route('admin.booking.edit', $bk->booking_id) }}"
                                    class="bg-indigo-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">EDIT</a>
                                <button onClick="destroy(this.id)" id="{{ $bk->booking_id }}"
                ...
                <div class="bg-white p-3">
                    {{ $booking->links('vendor.pagination.tailwind') }}
                </div>
                @endif
            </div>
        </div>
